...
{
	__pm_wakeup_event(&params->wake_src, UNPLUG_WAKE_PERIOD);
}
...
static int somc_chg_get_usbin_voltage(struct smbchg_chip *chip)
{
	...
	int usbin_voltage, rc = 0;

	if (IS_ERR_OR_NULL(params->usbin_vadc_dev)) {
		params->usbin_vadc_dev = iio_channel_get(chip->dev, "usbin");
	...
	rc = iio_read_channel_processed(params->usbin_vadc_dev, &usbin_voltage);
	if (rc) {
		dev_err(chip->dev, "Couldn't vadc_read(USBIN) rc=%d\n", rc);
		goto err;
	}

	return usbin_voltage;
...
static int somc_chg_register(struct smbchg_chip *chip)
{
	...
	wakeup_source_init(&params->wake_src, "unplug_wakelock");
...
static void somc_chg_unregister(struct smbchg_chip *chip)
{
	...
	wakeup_source_trash(&params->wake_src);
	if (params->apsd.wq)
