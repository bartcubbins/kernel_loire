...
static int get_prop_usb_dp_voltage_now(struct smbchg_chip *chip)
{
	...
	rc = iio_read_channel_processed(chip->usb_params.vadc_usb_dp,
				&dp_voltage_now);
	if (rc < 0) {
		pr_err("Unable to read usb_dp rc=%d\n", rc);
		return 0;
	} else {
		return dp_voltage_now;
	}
}

static int get_prop_usb_dm_voltage_now(struct smbchg_chip *chip)
{
	int dm_voltage_now, rc = 0;
	...
	rc = iio_read_channel_processed(chip->usb_params.vadc_usb_dm,
				&dm_voltage_now);
	if (rc < 0) {
		pr_err("Unable to read usb_dm rc=%d\n", rc);
		return 0;
	} else {
		return dm_voltage_now;
	}
}
