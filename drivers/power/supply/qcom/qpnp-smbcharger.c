...
struct smbchg_chip {
	...
	struct iio_channel		*vchg_adc_channel;
...
static int smbchg_get_iusb(struct smbchg_chip *chip)
{
	...
	if (!chip->vchg_adc_channel)
		return -EINVAL;

	rc = iio_read_channel_processed(chip->vchg_adc_channel,
			&iusb_ua);
	if (rc < 0) {
		pr_smb(PR_STATUS,
...
static int smbchg_hw_init(struct smbchg_chip *chip)
{
	...
	if (!IS_ERR_OR_NULL(chip->vchg_adc_channel)) {
...
#define TYPEC_PROBE_RETRY_MAX	5
	...
	chip->vchg_adc_channel = iio_channel_get(&pdev->dev, "vchg_sns");
	if (IS_ERR(chip->vchg_adc_channel)) {
		if (PTR_ERR(chip->vchg_adc_channel) == -EPROBE_DEFER)
			return -EPROBE_DEFER;
		dev_err(&pdev->dev, "vchg_sns channel is not available\n");
		chip->vchg_adc_channel = NULL;
	}
