		...
		struct device_attribute *attr, char *buf)
{
	...
	ret = panel_incell_get_power_status(&power_status);
...
static ssize_t panel_incell_intf_type_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	intf_mode = (panel_incell_intf_mode)(mode);
	pr_notice("%s: buf = %s\n", __func__, buf);
	rc = panel_incell_control_mode(intf_mode, INCELL_FORCE);
...
static ssize_t panel_incell_power_lock_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int rc = INCELL_OK, mode;
	panel_incell_pw_lock intf_lock;
	panel_incell_pw_status power_status;
	...
	intf_lock = (panel_incell_pw_lock)(mode);
	pr_notice("%s: buf = %s\n", __func__, buf);
	rc = panel_incell_power_lock_ctrl(mode, &power_status);
	pr_notice("%s: Incell execution %s\n", __func__,
			rc == INCELL_OK ? "suscess" : "fail");
	pr_notice("%s: return %d\n", __func__, rc);
	return count;
...
static ssize_t panel_incell_ewu_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int rc = INCELL_OK, mode;
	panel_incell_ewu_mode intf_ewu;
	...
	intf_ewu = (panel_incell_ewu_mode)(mode);
	pr_notice("%s: buf = %s\n", __func__, buf);
	panel_incell_ewu_mode_ctrl(intf_ewu);
	pr_notice("%s: Incell execution %s\n", __func__,
			rc == INCELL_OK ? "suscess" : "fail");
	pr_notice("%s: return %d\n", __func__, rc);
	return count;
...
static struct device_attribute incell_attributes[] = {
	...
	__ATTR(incell_intf, S_IWUSR|S_IWGRP, NULL, panel_incell_intf_type_store),
	__ATTR(incell_lock, S_IWUSR|S_IWGRP, NULL, panel_incell_power_lock_store),
	__ATTR(incell_ewu, S_IWUSR|S_IWGRP, NULL, panel_incell_ewu_store),
};
...
static ssize_t mdss_dsi_panel_frame_counter(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct fps_data fpsd = panel_driver_get_fps_data();
...
static ssize_t mdss_dsi_panel_vsyncs_per_ksecs_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	return panel_driver_vsyncs_per_ksecs_store(dev, buf, count);
}

static ssize_t mdss_dsi_panel_vsyncs_per_ksecs_show(struct device *dev,
			struct device_attribute *attr, char *buf)
{
	struct fps_data vpsd = panel_driver_get_vps_data();
...
static ssize_t mdss_dsi_panel_change_fpks_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	return panel_driver_change_fpks_store(dev, attr, buf, count);
}

static ssize_t mdss_dsi_panel_change_fpks_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return panel_driver_change_fpks_show(dev, attr, buf);
}

static ssize_t mdss_dsi_panel_change_fps_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	return panel_driver_change_fps_store(dev, attr, buf, count);
}

static ssize_t mdss_dsi_panel_change_fps_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return panel_driver_change_fps_show(dev, attr, buf);
}
