...
struct panel_incell_ctrl *incell;
struct panel_incell_ctrl incell_buf;
...
int panel_incell_get_power_status(panel_incell_pw_status *power_status)
{
	...
	if (panel_driver_is_power_on(incell->state)) {
...
static int panel_incell_driver_power_off(struct fb_info *info)
{
...
static int panel_incell_driver_send_power_on_seq(struct mdss_panel_data *pdata)
{
	int ret = INCELL_ERROR;
	...
	if (incell->panel_incell_intf_operation == INCELL_TOUCH_RUN &&
...
static int panel_incell_driver_send_power_on_fb(struct fb_info *info)
{
	int ret = INCELL_ERROR;
...
static int panel_incell_driver_power_on(struct fb_info *info)
{
	int ret = INCELL_ERROR;
	struct mdss_data_type *mdata = mdss_mdp_get_mdata();
	struct mdss_mdp_ctl *ctl = NULL;
	...
	if (mdss_fb_is_power_on(mfd))
		ret = panel_incell_driver_send_power_on_seq(pdata);
	else
		ret = panel_incell_driver_send_power_on_fb(info);
...
static int panel_incell_display_hw_reset(struct panel_incell_ctrl *incell,
				struct fb_info *info)
{
	int ret = INCELL_ERROR;
	struct mdss_data_type *mdata = mdss_mdp_get_mdata();
	...
	if (panel_driver_is_power_on(incell->state))
		 ...
		 * the case of FB off.
		 */
		ret = panel_incell_driver_power_off(info);
	else
		pr_debug("%s: Skip LCD off 0x%x\n", __func__,
					(incell->state));

	if (!panel_driver_is_system_on(incell->state)
	&& !panel_driver_is_power_on(incell->state)) {
...
static int panel_incell_display_off(struct panel_incell_ctrl *incell,
				struct fb_info *info)
{
	int ret = INCELL_ERROR;
...
int panel_incell_control_mode(panel_incell_intf_mode mode, bool force)
{
	 ...
	 */
	if (panel_driver_is_power_lock(incell->state)) {
	...
	if (!mutex_trylock(&info->lock)) {
		incell->panel_incell_intf_operation = INCELL_TOUCH_IDLE;
	...
	case INCELL_DISPLAY_HW_RESET:
		...
		ret = panel_incell_display_hw_reset(incell, info);
		spec_mfd->off_sts = false;
		break;
	case INCELL_DISPLAY_OFF:
		spec_mfd->off_sts = true;
		ret = panel_incell_display_off(incell, info);
		break;
	default:
...
static void panel_incell_panel_power_worker(struct work_struct *work)
{
	struct panel_incell_ctrl *incell = panel_incell_get_info();
...
static void panel_incell_panel_power_worker_scheduling(panel_incell_pw_lock lock,
						struct panel_incell_ctrl *incell)
{
...
static int panel_incell_power_lock(unsigned char *state)
{
		...
		panel_incell_pw_status *power_status)
{
	...
	if (lock == INCELL_DISPLAY_POWER_LOCK)
		ret = panel_incell_power_lock(&(incell->state));
	else
		ret = incell_power_unlock(&(incell->state));

	pr_debug("%s: ---> status:0x%x\n", __func__, (incell->state));

	panel_incell_get_power_status(power_status);
	panel_incell_panel_power_worker_scheduling(lock, incell);
...
{
	memset(&incell_buf, 0, sizeof(struct panel_incell_ctrl));
	incell = &incell_buf;
	...
	INIT_WORK(&incell->incell_work, panel_incell_panel_power_worker);
}
