		...
		struct mdss_dsi_ctrl_pdata *ctrl_pdata, char *vreg, bool enable)
{
	...
	valid = panel_driver_vreg_name_to_config(
...
bool panel_driver_is_power_on(unsigned char state)
{
	bool ret = false;
...
bool panel_driver_is_power_lock(unsigned char state)
{
	bool ret = false;
...
bool panel_driver_is_ewu(unsigned char state)
{
	bool ret = false;
...
bool panel_driver_is_system_on(unsigned char state)
{
	bool ret = false;
...
static bool panel_driver_is_seq_for_ewu(void)
{
...
static bool panel_driver_is_incell_operation(void)
{
	struct panel_incell_ctrl *incell = panel_incell_get_info();

	if (incell &&
		(incell->panel_incell_intf_operation == INCELL_TOUCH_RUN))
		return true;

	return false;
}

static int panel_calculation_sleep(
		struct mdss_dsi_ctrl_pdata *ctrl_pdata,
...
static void panel_driver_gpio_output(
		struct mdss_dsi_ctrl_pdata *ctrl_pdata,
		int gpio, bool enable, int value)
{
	int wait;

	wait = panel_calculation_sleep(ctrl_pdata, gpio, enable);
...
static void panel_driver_set_gpio(
		struct mdss_dsi_ctrl_pdata *ctrl_pdata,
		int gpio, bool enable, int value)
{
	int wait = 0;
...
int panel_driver_pinctrl_init(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
...
int panel_driver_touch_pinctrl_set_state(
	struct mdss_dsi_ctrl_pdata *ctrl_pdata,
	bool active)
{
	...
	if (!IS_ERR_OR_NULL(pin_state)) {
		rc = pinctrl_select_state(ctrl_pdata->pin_res.pinctrl,
				pin_state);
		if (!rc) {
			wait = panel_calculation_sleep(ctrl_pdata,
...
void panel_driver_state_change_off(struct panel_incell_ctrl *incell)
{
	panel_incell_state_change change_state = incell->change_state;
	unsigned char *state = &incell->state;

	pr_debug("%s: status:0x%x --->\n", __func__, (*state));

	if (change_state != INCELL_STATE_NONE)
		panel_driver_update_incell_bk(incell);
...
void panel_driver_power_off_ctrl(struct panel_incell_ctrl *incell)
{
	panel_incell_pw_seq seq = POWER_OFF_EXECUTE;
	panel_incell_state_change change_state = INCELL_STATE_NONE;
	unsigned char state = incell->state;
	panel_incell_intf_mode intf_mode = incell->intf_mode;
	panel_incell_worker_state worker_state = incell->worker_state;
	bool incell_intf_operation = incell->panel_incell_intf_operation;
	...
	if (worker_state == INCELL_WORKER_ON) {
		...
		if (intf_mode == INCELL_DISPLAY_HW_RESET) {
			...
			if (!panel_driver_is_power_on(state)) {
				pr_err("%s: Power off status. state:0x%x\n",
						__func__, state);
				seq = POWER_OFF_SKIP;
			} else if (panel_driver_is_ewu(state)) {
		...
		if (worker_state == INCELL_WORKER_PENDING)
			panel_incell_panel_power_worker_canceling(incell);

		/* touch I/F idling mode */
		if (panel_driver_is_power_lock(state)) {
			change_state = INCELL_STATE_S_OFF;
			seq = POWER_OFF_SKIP;
		} else if (!panel_driver_is_power_on(state)) {
			change_state = INCELL_STATE_S_OFF;
			seq = POWER_OFF_SKIP;
		} else if (panel_driver_is_ewu(state)) {
			change_state = INCELL_STATE_S_OFF;
...
int panel_driver_power_off(struct mdss_panel_data *pdata)
{
	...
	ret += panel_driver_reset_touch(pdata, 0);
	ret += panel_driver_vreg_ctrl(ctrl_pdata, "ibb", false);
	ret += panel_driver_vreg_ctrl(ctrl_pdata, "lab", false);

	panel_driver_set_gpio(ctrl_pdata,
	...
	panel_driver_touch_pinctrl_set_state(ctrl_pdata, false);

	panel_driver_set_gpio(ctrl_pdata,
		(spec_pdata->touch_vddio_gpio), false, 1);

	ret += panel_driver_vreg_ctrl(ctrl_pdata, "vddio", false);

	ret += panel_driver_vreg_ctrl(ctrl_pdata, "touch-avdd", false);

	if (ret)
		pr_err("%s: failed to disable vregs for %s\n",
				__func__, __mdss_dsi_pm_name(DSI_PANEL_PM));
	else
		pr_notice("%s: -----display panel power off-----\n",
				  __func__);
...
void panel_driver_state_change_on(struct panel_incell_ctrl *incell)
{
	panel_incell_state_change change_state = incell->change_state;
	unsigned char *state = &incell->state;
...
void panel_driver_power_on_ctrl(struct panel_incell_ctrl *incell)
{
	panel_incell_pw_seq seq = POWER_ON_EXECUTE;
	panel_incell_state_change change_state = INCELL_STATE_NONE;
	unsigned char state = incell->state;
	panel_incell_intf_mode intf_mode = incell->intf_mode;
	panel_incell_worker_state worker_state = incell->worker_state;
	...
	if (worker_state == INCELL_WORKER_ON) {
		...
		} else if (panel_driver_is_power_lock(state)) {
			if (panel_driver_is_power_on(state)) {
				change_state = INCELL_STATE_S_ON;
				seq = POWER_ON_SKIP;
			} else {
...
static int panel_driver_ewu_seq(struct mdss_panel_data *pdata)
{
	int ret = 0;
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	...
	ret += panel_driver_reset_touch(pdata, 0);
	ret += panel_driver_reset_dual_display(ctrl_pdata);

	return ret;
}

int panel_driver_power_on(struct mdss_panel_data *pdata)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	struct mdss_panel_specific_pdata *spec_pdata = NULL;
	struct mdss_panel_power_seq *pw_seq = NULL;
	...
	if (incell) {
		panel_driver_power_on_ctrl(incell);
		if (incell->seq != POWER_ON_EXECUTE) {
			if (incell->seq == POWER_ON_EWU_SEQ)
				ret = panel_driver_ewu_seq(pdata);
			return ret;
		}
		state = incell->state;
	}
	...
	ret += panel_driver_vreg_ctrl(ctrl_pdata, "touch-avdd", true);
	ret += panel_driver_vreg_ctrl(ctrl_pdata, "vddio", true);

	panel_driver_gpio_output(ctrl_pdata,
		(spec_pdata->touch_vddio_gpio), true, 0);

	panel_driver_touch_pinctrl_set_state(ctrl_pdata, true);

	panel_driver_gpio_output(ctrl_pdata,
		(spec_pdata->disp_dcdc_en_gpio), true, 1);

	if ((spec_pdata->panel_type == HYBRID_INCELL) &&
	    (!panel_driver_is_power_on(state))) {
		ret += panel_driver_reset_touch(pdata, 1);
		wait = pw_seq->touch_reset_first;
		usleep_range(wait * 1000, wait * 1000 + 100);
	}

	ret += panel_driver_vreg_ctrl(ctrl_pdata, "lab", true);
	ret += panel_driver_vreg_ctrl(ctrl_pdata, "ibb", true);
	...
	pr_notice("%s: -----display panel power on-----\n",
			  __func__);;
...
int panel_driver_request_gpios(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	struct mdss_panel_specific_pdata *spec_pdata = NULL;
	int rc = 0;
...
void panel_driver_gpio_free(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	struct mdss_panel_specific_pdata *spec_pdata = NULL;
...
void panel_driver_parse_gpio_params(struct platform_device *ctrl_pdev,
		struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	struct mdss_panel_specific_pdata *spec_pdata = NULL;
...
static void panel_set_gpio_seq(
...
int panel_driver_reset_panel(struct mdss_panel_data *pdata, int enable)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	...
	panel_set_gpio_seq(ctrl_pdata->rst_gpio,
				pw_seq->seq_num, pw_seq->rst_seq);

	return rc;
}

int panel_driver_reset_touch(struct mdss_panel_data *pdata, int enable)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
	struct mdss_panel_info *pinfo = NULL;
	struct mdss_panel_specific_pdata *spec_pdata = NULL;
...
static bool panel_driver_split_display_enabled(void)
{
	/*
...
int panel_driver_reset_dual_display(
			struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	struct mdss_dsi_ctrl_pdata *mctrl_pdata = NULL;
	int ret = 0;

	if (!panel_driver_split_display_enabled()) {
		if (mdss_dsi_pinctrl_set_state(ctrl_pdata, true))
			pr_debug("reset enable: pinctrl not enabled\n");
...
int panel_driver_parse_dt(struct device_node *np,
		struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	struct mdss_panel_specific_pdata *spec_pdata = NULL;
	struct mdss_panel_info *pinfo = NULL;
...
int panel_pcc_setup(struct mdss_panel_data *pdata)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = NULL;
...
static struct fps_data *panel_driver_get_fps_address(fps_type type)
{
	switch (type) {
	case FPSD:
		return &fpsd;
...
static void panel_driver_fps_data_init(fps_type type)
{
	struct fps_data *fps = panel_driver_get_fps_address(type);
...
void panel_driver_fps_data_update(
...
struct fps_data panel_driver_get_fps_data(void)
{
	return fpsd;
}

struct fps_data panel_driver_get_vps_data(void)
{
	return vpsd;
}
...
static void vsync_handler(struct mdss_mdp_ctl *ctl, ktime_t t)
{
	...
	panel_driver_fps_data_update(mfd, VPSD);
}

static void panel_driver_vsync_handler_init(void)
{
	vs_handle.vsync_handler = NULL;
}

ssize_t panel_driver_vsyncs_per_ksecs_store(struct device *dev,
...
static void panel_driver_fps_cmd_send(
		struct mdss_dsi_ctrl_pdata *ctrl_pdata,
...
static int panel_driver_fps_calc_rtn(
	...
	panel_driver_fps_cmd_send(ctrl_pdata, dfpks_rev, dfpks);

	return 0;
}

static int panel_driver_fps_calc_porch
		(struct mdss_dsi_ctrl_pdata *ctrl_pdata, int dfpks) {
	u64 vmclk;
	u64 vtouch;
	u32 dfpks_rev;
...
static int panel_driver_fps_calc_adj
		(struct mdss_dsi_ctrl_pdata *ctrl_pdata, int dfpks) {
	u32 dfpks_rev;
...
static int panel_chg_fps_calc
	...
	switch (spec_pdata->chg_fps.type) {
	case FPS_TYPE_UHD_4K:
		ret = panel_driver_fps_calc_adj(ctrl_pdata, dfpks);
		break;
	case FPS_TYPE_HYBRID_INCELL:
		ret = panel_driver_fps_calc_porch(ctrl_pdata, dfpks);
		break;
	case FPS_TYPE_FULL_INCELL:
		ret = panel_driver_fps_calc_rtn(ctrl_pdata, dfpks);
		break;
	default:
...
static int panel_chg_fps_check_state
	...
	if (mdss_dsi_sync_wait_enable(ctrl)) {
		...
		if (sctrl) {
			if (mdss_dsi_sync_wait_trigger(ctrl)) {
				rc = panel_chg_fps_calc(sctrl, dfpks);
				if (rc < 0)
					goto end;
				rc = panel_chg_fps_calc(ctrl, dfpks);
			} else {
				rc = panel_chg_fps_calc(ctrl, dfpks);
				if (rc < 0)
					goto end;
...
ssize_t panel_driver_change_fpks_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	rc = panel_chg_fps_check_state(ctrl_pdata, dfpks);
	if (rc) {
		pr_err("%s: Error, rc = %d\n", __func__, rc);
		return rc;
	}
...
ssize_t panel_driver_change_fpks_show(struct device *dev,
...
ssize_t panel_driver_change_fps_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = dev_get_drvdata(dev);
	int dfps, dfpks, rc;
...
ssize_t panel_driver_change_fps_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct mdss_dsi_ctrl_pdata *ctrl_pdata = dev_get_drvdata(dev);
	struct mdss_data_type *mdata = mdss_mdp_get_mdata();
...
void panel_driver_check_splash_enable(
		struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
...
static void panel_driver_chg_fps_cmds_send
			(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	struct mdss_panel_specific_pdata *spec_pdata = NULL;
...
void panel_driver_fb_notifier_call_chain(
		struct msm_fb_data_type *mfd, int blank, bool type)
{
	...
	if ((mfd->panel_info->type == MIPI_VIDEO_PANEL) ||
		...
		if (!panel_driver_is_incell_operation()) {
			event.info = mfd->fbi;
			event.data = &blank;
...
void panel_driver_labibb_vreg_init(
		struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	int ret;
	int min_uV, max_uV = 0;
	 ...
	 * Get lab/ibb info.
	 */
	ret = panel_driver_vreg_name_to_config(ctrl_pdata,
						&lab_vreg_config, "lab");
	if (ret) {
...
void panel_driver_init(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	ctrl_pdata->spec_pdata->pcc_setup = panel_pcc_setup;
	...
	panel_driver_fps_data_init(FPSD);
	panel_driver_fps_data_init(VPSD);
	panel_driver_vsync_handler_init();
}

void panel_driver_off(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	struct mdss_data_type *mdata = mdss_mdp_get_mdata();
	struct mdss_mdp_ctl *ctl = mdata->ctl_off;
...
void panel_driver_post_on(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
	struct mdss_panel_specific_pdata *spec_pdata = NULL;
	struct mdss_panel_data *pdata;
	...
	if (spec_pdata->chg_fps.enable) {
		if (spec_pdata->chg_fps.mode == FPS_MODE_SUSRES)
			panel_chg_fps_calc(ctrl_pdata,
						spec_pdata->input_fpks);

		panel_driver_chg_fps_cmds_send(ctrl_pdata);
	} else {
...
void panel_driver_unblank(struct mdss_dsi_ctrl_pdata *ctrl_pdata)
{
...
void panel_driver_dump_incell_sts(struct panel_incell_ctrl *incell)
{
	int num;
...
void panel_driver_update_incell_bk(struct panel_incell_ctrl *incell)
{
	int num;
