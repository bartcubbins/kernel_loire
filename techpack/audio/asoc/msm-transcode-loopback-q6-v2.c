...
static int __init msm_transcode_loopback_init(void)
{
...
module_init(msm_transcode_loopback_init);

static void __exit msm_transcode_loopback_exit(void)
{
	mutex_destroy(&transcode_info.lock);
	platform_driver_unregister(&msm_transcode_loopback_driver);
}
module_exit(msm_transcode_loopback_exit);
